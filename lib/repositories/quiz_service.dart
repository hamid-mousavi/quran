...
class QuizService {
  ...
  Future<List<Map<String, dynamic>>> generateRandomQuestions({
    ...
    List<QuranText> verses =
        await _getRandomVerses(level, questionCount, startPage, endPage);
  ...
  Future<List<QuranText>> _getRandomVerses(
      String level, int count, int startPage, int endPage) async {
    Random rand = Random();
    List<QuranText> verses = [];
    var pages = List.generate(endPage - startPage + 1,
        (index) => startPage + index); // صفحات مشخص‌شده
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (verseList.isNotEmpty)
        verses.add(verseList[rand.nextInt(verseList.length)]);
    }
    return verses;
  }

  Future<Map<String, dynamic>> _createTranslationQuestion(
      QuranText verse, String questionType) async {
  ...
  Future<Map<String, dynamic>> _createPreviousNextQuestion(
      QuranText verse, String questionType) async {
    var prevVerse =
  ...
  Future<Map<String, dynamic>> _createSuraStartQuestion(
      QuranText verse, String questionType) async {
  ...
  Future<List<String>> _getWrongAnswers(int sura, int aya) async {
    ...
    for (var i = 0; i < 3; i++) {
      ...
      var wrongVerse =
          await quranService.getVerseBySuraAya(randomSura, randomAya);
